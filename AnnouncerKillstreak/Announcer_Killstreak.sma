...
new const SOUND_PACK_FOLDER[] = "Khepri";
...
const Float: MULTIKILL_TIME = 9.0;

// Режим HUD'а
#define COLOR_MODE 3 // 1 - Одноцветный; 2 - Многоцветный; 3 - TeamColor; 4 - Случайный
...
// Грани рандомизации цвета HUD при COLOR_MODE 4
stock const MIN_VALUE = 50;
stock const MAX_VALUE = 255;

// TeamColor COLOR_MODE 3
stock const HUD_TE_R = 240;
stock const HUD_TE_G = 128;
stock const HUD_TE_B = 128;
stock const HUD_CT_R = 135;
stock const HUD_CT_G = 206;
stock const HUD_CT_B = 235;
...
#define SAVE_TYPE 1 // 0 - nVault; 1 - Trie
...
public plugin_init(){
	...
	register_clcmd("say /sound", "Command_Announce");
	register_clcmd("say_team /sound", "Command_Announce");
...
public Command_Announce(iPlayer){
	...
	client_print( iPlayer, print_chat, ^"^3[^2AK^3] %L %L", iPlayer, ^"AK_COMMAND",
		iPlayer, g_szBuffer );
...
public CBasePlayer_Killed_Pre(iVictim, iKiller){
	...
	new const PlayersCount = get_playersnum_ex(GetPlayers_ExcludeHLTV | GetPlayers_ExcludeBots);
	
	if(!g_bFirstBlood && PlayersCount >= 4){
...
#if COLOR_MODE == 1
	set_hudmessage(HUD_R, HUD_G, HUD_B, HUD_X, HUD_Y, 0, 0.0, HUD_TIME, .channel = 3);
#elseif COLOR_MODE == 2
	set_hudmessage(g_iColor[iPos][R], g_iColor[iPos][G], g_iColor[iPos][B], HUD_X, HUD_Y, 0, 0.0, HUD_TIME, .channel = 3);
#elseif COLOR_MODE == 3
	if(get_user_team(g_iKiller) == 1)
		set_hudmessage(HUD_TE_R, HUD_TE_G, HUD_TE_B, HUD_X, HUD_Y, 0, 0.0, HUD_TIME, .channel = 3);
	else
		set_hudmessage(HUD_CT_R, HUD_CT_G, HUD_CT_B, HUD_X, HUD_Y, 0, 0.0, HUD_TIME, .channel = 3);
#else
	set_hudmessage(	random_num(MIN_VALUE, MAX_VALUE), random_num(MIN_VALUE, MAX_VALUE),
		random_num(MIN_VALUE, MAX_VALUE), HUD_X, HUD_Y, 0, 0.0, HUD_TIME, .channel = 3 );
#endif
