...
public plugin_init()
{
	...
	register_clcmd("banreason", "clcmdBanReason",	ADMIN_BAN | ADMIN_BAN_TEMP);
	register_clcmd("amx_banmenu", "clcmdBanMenu",	ADMIN_BAN | ADMIN_BAN_TEMP);
	register_clcmd("amx_offbanmenu", "clcmdOffBanMenu", ADMIN_BAN | ADMIN_BAN_TEMP);
	register_clcmd("amx_unbanmenu", "clcmdUnBanMenu", ADMIN_BAN | ADMIN_BAN_TEMP);
	
	register_concmd("amx_ban",	"concmdBan",	ADMIN_BAN | ADMIN_BAN_TEMP);
	register_concmd("amx_unban","concmdUnBan",		ADMIN_BAN | ADMIN_BAN_TEMP);
	register_concmd("find_ban", "concmdFindBan",	ADMIN_BAN | ADMIN_BAN_TEMP);
	
	register_clcmd("say /bans", "clcmdBanList");
	register_clcmd("say_team /bans", "clcmdBanList");
	
	g_pCvars[host] = register_cvar("lb_sql_host",	"");
	g_pCvars[user] = register_cvar("lb_sql_user",	"",		FCVAR_PROTECTED);
	g_pCvars[pass] = register_cvar("lb_sql_pass",	"", FCVAR_PROTECTED);
	g_pCvars[db]   = register_cvar("lb_sql_db",		"");
	g_pCvars[pref] = register_cvar("lb_sql_pref",	"");
...
public plugin_cfg()
{
	...
	new str[190]; 
	formatex(str, charsmax(str), "%L", LANG_PLAYER, "TIMEMENU_TITLE");
	g_iTimeMenu		= menu_create(str, "TimeMenuHandler");
	formatex(str, charsmax(str), "%L", LANG_PLAYER, "REASONMENU_TITLE");
...
public clcmdBanMenu(id, flags)
{
	...
	new str[190]; 
	formatex(str, charsmax(str), "%L", LANG_PLAYER, "BANMENU_TITLE");
...
public clcmdOffBanMenu(id, flags)
{
	...
	new str[190]; 
	formatex(str, charsmax(str), "%L", LANG_PLAYER, "OFFMENU_TITLE");
...
public clcmdBanList(id)
{
	formatex(szQuery, charsmax(szQuery),
		"SELECT `player_nick`, `admin_nick`, `ban_reason`, `ban_created`, `ban_length` FROM `%s_bans` WHERE `expired` = '0' ORDER BY `bid` DESC LIMIT 15", g_szTablePrefix);
...
public SQL_Handler(failstate, Handle:query, err[], errcode, dt[], datasize)
{
	...
	switch(dt[0])
	{
		case Unban: UTIL_console_print(id, " [LB] %L", id, SQL_AffectedRows(query) ? "CLIENT_UNBANNED" : "CLIENT_NOT_FOUND");
		case Check:
		{
		...
		{
			if(!SQL_NumResults(query))
				UTIL_console_print(id, " [LB] %L", id, "BAN_NOT_FOUND");
			else
			{
		...
		case Ban:
		{
			...
			set_task((fDelay < 1.0) ? 1.0 : fDelay, "Task__Kick", id);
		}
		case UnbanMenu:
		{
			if(!SQL_NumResults(query))
				UTIL_console_print(id, " [LB] %L", id, "UNBANMENU_PLAYERS_NOT_FOUND");
			else
			{
				new str[190]; 
				formatex(str, charsmax(str), "%L", LANG_PLAYER, "UNBANMENU_TITLE");
				...
				if(get_user_flags(id) & g_Cvars[superadmin])
				{
					while(SQL_MoreResults(query))
					{
						...
						formatex(szMenuItem, charsmax(szMenuItem), ^"%s ^0[\y%s^0]", szName, szAdmin);
		...
		case BanList:
		{
			if(SQL_NumResults(query))
			{
				static motd[2048], title[32], adminTitle[32], reasonTitle[16], expireTitle[16], motdLength, place, szName[MAX_NAME_LENGTH * 2], szAdmin[MAX_NAME_LENGTH * 2], szReason[64], szBanExp[64], iBanLen;

				motdLength = 0, place = 0;

				formatex(title, charsmax(title), "%L", id, "BANLIST_TITLE");
				formatex(adminTitle, charsmax(adminTitle), "%L", id, "BANLIST_ADMIN");
				formatex(reasonTitle, charsmax(reasonTitle), "%L", id, "BANLIST_REASON");
				formatex(expireTitle, charsmax(expireTitle), "%L", id, "BANLIST_EXPIRE");

				motdLength = format(motd, charsmax(motd), ^"^0%1s ^3%s%s", "#", title, "^n");
				...
				while(SQL_MoreResults(query))
				{
					...
					SQL_ReadResult(query, 0, szName, charsmax(szName));
					SQL_ReadResult(query, 1, szAdmin, charsmax(szAdmin));
					SQL_ReadResult(query, 2, szReason, charsmax(szReason));
					iBanLen = SQL_ReadResult(query, 4);
					...
					else
					{
						...
						get_time_length(
							id, 
							SQL_ReadResult(query, 3) + iBanLen * 60, 
							timeunit_seconds, 
							szBanExp, 
							charsmax(szBanExp)
						);
						*/
						format_time(szBanExp, charsmax(szBanExp), "%d.%m.%Y - %H:%M:%S", SQL_ReadResult(query, 3) + iBanLen * 60);
					}
					motdLength += format(motd[motdLength], charsmax(motd) - motdLength, ^"^0%1i ^7%s ^0[ ^3%s: ^1%s ^3%s: ^6%s ^0] ^7%s ^5%s%s", place, szName, reasonTitle, szReason, expireTitle, szBanExp, adminTitle, szAdmin, "^n");
...
BanAction(admin, banned)
{
	...
	new szIp[MAX_IP_LENGTH], szAuth[MAX_AUTHID_LENGTH], szaIP[MAX_IP_LENGTH], szaAuth[MAX_AUTHID_LENGTH];
	...
	if(admin)
	{
		get_user_ip(admin, szaIP, charsmax(szaIP), 1);
	...
	log_amx("Ban: ^"%s<%d><%s><>^" ban and kick ^"%s<%d><%s><>^" (minutes ^"%d^") (reason ^"%s^")", szaName, get_user_userid(admin), szaAuth, szuName, get_user_userid(banned), szAuth, g_arrBanData[admin][bantime], g_arrBanData[admin][reason]);
	
	new szBanLen[64];
	if(!g_arrBanData[admin][bantime]) {
		for (new i = 1; i <= MaxClients; i++)
			show_activity_id(i, admin, szaName, ^"%L %s^7 %L ^7(%L: ^1%s^7)", i, "BAN", szuName, i, "PERM", i, "REASON", g_arrBanData[admin][reason]);
			
		formatex(szBanLen, charsmax(szBanLen), "%L", LANG_PLAYER, "BAN_PERMANENT");
	}
	else {
		new tempTime[32];
		num_to_str(g_arrBanData[admin][bantime], tempTime, charsmax(tempTime));
		for (new i = 1; i <= MaxClients; i++)
			show_activity_id(i, admin, szaName, ^"%L %s^7 %L ^7(%L: ^1%s^7)", i, "BAN", szuName, i, "FOR_MIN", tempTime, i, "REASON", g_arrBanData[admin][reason]);
			
		get_time_length(banned, g_arrBanData[admin][bantime], timeunit_minutes, szBanLen, charsmax(szBanLen));
	}
	
	UTIL_console_print(admin, " [LB] %L", admin, "CLIENT_BANNED", szuName);
	...
	if(HudSyncObj || (HudSyncObj = CreateHudSyncObj()))
	{
		...
		new szText[512];
		formatex(szText, charsmax(szText), "%L", LANG_PLAYER, "HUD_MSG_BAN", szuName, szBanLen, szaName, g_arrBanData[admin][reason]);
...
{
	new szaName[64], szaIP[MAX_IP_LENGTH], szaAuth[MAX_AUTHID_LENGTH];
...
LoadConfigs()
{
	...
	if(g_Cvars[static_reasons])
	{
		...
		if(get_pcvar_num(g_pCvars[crsn]))
		{
			...
			new str[64]; formatex(str, charsmax(str), ^"^1%L", LANG_PLAYER, "CUSTOM_REASON");
