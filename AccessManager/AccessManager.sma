...
func_AuthPlayer(pPlayer) {
	...
	for(new i; i < g_iAccCount; i++) {
		...
		if(bitAuthFlags & FLAG_AUTHID) {
			if(
				szAuthID[11] != g_eUserData[ACCOUNT_DATA__AUTHID][11] // VALVE_XASH_057ea1b40be40469ae9366af21d9557f
					||
...
public client_infochanged(pPlayer) {
	if(is_user_connected(pPlayer)) {
		...
		get_user_info(pPlayer, "name", szNewName, charsmax(szNewName))
		get_user_name(pPlayer, szOldName, charsmax(szOldName))
	
		if(!equali(szNewName, szOldName)) {
			RequestFrame("func_AuthDelay", pPlayer)
		}
	}
}
...
public concmd_ReloadAdmins(pPlayer, bitAccess) {
	...
	console_print(pPlayer, "%l", "ACCESS_MANAGER__RELOADING_ADMINS")
		...
		if(iRowIdToExtend) {
			...
			formatex( g_szQuery, chx(g_szQuery),
				"INSERT INTO `amx_amxadmins` \
					(`username`,`access`,`flags`,`xashid`,`nickname`,`ashow`,`created`,`expired`,`days`,`ingame`) \
						VALUES \
