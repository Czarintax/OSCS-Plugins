...
public CBasePlayer_Killed(const index, const pevAttacker, iGib)
{
	if(!pevAttacker || index == pevAttacker)
		return HC_CONTINUE
#if defined ONLY_LAST_PLAYER
	if(!is_last_player(index))
		return HC_CONTINUE
#endif
...
#if defined ONLY_LAST_PLAYER
stock bool:is_last_player(const player)
{
    new playerTeam = get_member(player, m_iTeam)
    new aliveCount = 0
    
    for(new i = 1; i <= MaxClients; i++)
    {
        if(!is_user_alive(i) || i == player)
            continue
            
        if(get_member(i, m_iTeam) == playerTeam)
            aliveCount++
    }
