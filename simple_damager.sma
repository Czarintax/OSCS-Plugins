...
@CBasePlayer_TakeDamage_Post(id, inflictor, attacker, Float:damage, bitsDamageType)
{
	...
	if(damage <= 1.0 || !rg_is_player_can_takedamage(id, attacker) || id == attacker)
		return;
	...
	if (!damager_attacker_flags || get_user_flags(attacker) & damager_attacker_flags)
	{
		...
		set_hudmessage(color[0], color[1], color[2], position[0], position[1], 2, 0.1, 1.5, 0.01, 0.01, .channel = 2);
