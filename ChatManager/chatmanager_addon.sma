...
public cm_player_send_message(id, message[])
{
	...
	{
		client_print(id, print_chat, ^"%L", id, "CM_ADDON_WARN_MIN_MSG_DELAY");
		add_warning(id);
		return MESSAGE_BLOCKED;
	}
	...
	if(equal(message, g_sLastMessage[id]))
	{
		...
		{
			client_print(id, print_chat, ^"%L", id, "CM_ADDON_WARN_MAX_IDENTICAL_MSG");
			add_warning(id);
			return MESSAGE_BLOCKED;
		}
	}
	...
	{
		client_print(id, print_chat, ^"%L", id, "CM_ADDON_WARN_MAX_CAPS_PERCENT");
		add_warning(id);
		return MESSAGE_BLOCKED;
	}
	...
	if(regex_match_c(message, g_rIpPattern, ret))
	{
		...
		{
			client_print(id, print_chat, ^"%L", id, "CM_ADDON_WARN_BLOCKED_IP");
			add_warning(id);
			return MESSAGE_BLOCKED;
		}
	}
	if(regex_match_c(message, g_rDomainPattern, ret))
	{
		...
		{
			client_print(id, print_chat, ^"%L", id, "CM_ADDON_WARN_BLOCKED_DOMAIN");
			add_warning(id);
			return MESSAGE_BLOCKED;
		}
	}
...
add_warning(id)
{
	if(++g_iWarnings[id] >= MAX_WARNINGS_TO_BLOCK_CHAT)
	{
		...
		g_iWarnings[id] = 0;
		client_print(id, print_chat, ^"%L", id, "CM_ADDON_WARN_BLOCKED_CHAT", BLOCK_CHAT_TIME);
	}
