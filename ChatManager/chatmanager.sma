...
#if defined FUNCTION_CSGO_RANK
native csgo_get_rank(id);
new const CSGO_RANK_FORMAT[] = ^"^%s^7 ";
new const CSGO_RANKS[18 + 1][] = {
	"7{Unranked}",
	"7{Silver I}",
	"7{Silver II}",
	"7{Silver III}",
...
public clcmd__say_handler(id)
{
	...
	replace_all(message, charsmax(message), ^"^0", "");
	...
	replace_all(message, charsmax(message), ^"^5", "");
	replace_all(message, charsmax(message), ^"^6", "");
	replace_all(message, charsmax(message), ^"^7", "");
	replace_all(message, charsmax(message), ^"^8", "");
	replace_all(message, charsmax(message), ^"^9", "");

	new name_color[3];
	switch(rg_get_user_team(id)) {
		case 1: name_color = ^"^1";
		case 2: name_color = ^"^5";
		default: name_color = ^"^7";
	}
	...
	new TeamName:sender_team = rg_get_user_team(id);
	...
	new players[32], players_num, player, is_player_alive, TeamName:player_team, player_flags;
	...
	#if defined FUNCTION_LOG_MESSAGES
	static const team_color[TeamName][] = {"gray", "red", "blue", "gray"};
	new log_msg[256];
...
public FormatMessage(sender, TeamName:sender_team, channel, name_color[], chat_color[], time_code[], name[], message[])
{
	new text[MESSAGE_LENGTH], len = 1;
	text[0] = '^7';
	
	if(channel % 2) {
		len += formatex(text[len], charsmax(text) - len, "%s*%L*", name_color, LANG_SERVER, channel != 7 ? "CM_CHAN_DEAD" : "CM_CHAN_SPEC");
	}
	
	if(channel > 1 && channel < 7) {
		len += formatex(text[len], charsmax(text) - len, "%s(%L) ", name_color, LANG_SERVER, TEAM_NAMES[sender_team]);
	} else if(channel) {
	...
	#if defined FUNCTION_CSGO_RANK
	new rank_id = csgo_get_rank(sender);
	len += formatex(text[len], charsmax(text) - len, CSGO_RANK_FORMAT, CSGO_RANKS[rank_id]);
	#endif
	...
	len += formatex(text[len], charsmax(text) - len, ^"%s%s^7 :%s %s", name_color, name, chat_color, message);
...
public message__say_text(msgid, dest, receiver)
{
	...
	client_print(receiver, 3, g_sMessage);
...
stock get_user_text_channel(is_sender_alive, is_team_msg, TeamName:sender_team)
{
	if (is_team_msg) {
		switch(sender_team) {
			case TEAM_TERRORIST: {
	...
	return is_sender_alive ? 0 : (sender_team == TEAM_SPECTATOR ? 7 : 1);
}
